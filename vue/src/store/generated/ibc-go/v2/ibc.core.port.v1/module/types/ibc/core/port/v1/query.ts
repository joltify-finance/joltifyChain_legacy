...
const baseQueryAppVersionRequest: object = { port_id: '', connection_id: '', ordering: 0, proposed_version: '' }

export const QueryAppVersionRequest = {
  encode(message: QueryAppVersionRequest, writer: Writer = Writer.create()): Writer {
    if (message.port_id !== '') {
...
export const QueryAppVersionResponse = {
  encode(message: QueryAppVersionResponse, writer: Writer = Writer.create()): Writer {
    if (message.port_id !== '') {
      writer.uint32(10).string(message.port_id)
    }
    if (message.version !== '') {
...
export class QueryClientImpl implements Query {
  ...
  AppVersion(request: QueryAppVersionRequest): Promise<QueryAppVersionResponse> {
    ...
    return promise.then((data) => QueryAppVersionResponse.decode(new Reader(data)))
  }
}
